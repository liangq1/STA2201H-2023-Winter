...
stan_data=list(y=observe.i,
               ...
               N=length(observe.i),
               x=aff.i-mean(aff.i))
mod1 = stan(data=stan_data,file = "mod1_lab9.stan")
```
```{r}
mod1
```
...
## Model 2 
```{r, results="hide"}
mod2 = stan(data=stan_data,file = "mod2_lab9.stan")
```
...
## Model 3
```{r, results="hide"}
mod3 = stan(data=stan_data,file = "mod3_lab9.stan")
```
...
## Plot 1: A histogram of log theta estimates for model 1, model 2 and model 3
...
## Plot 2: Log estimates for each region are presented in the graph, along with error bars. The estimates for each model are color-coded, and the size of the points reflects the number of deaths observed in each region
...
res %>% 
  ...
  geom_errorbar(aes(ymin=lower_mod1,ymax=upper_mod1),alpha=0.6)+
